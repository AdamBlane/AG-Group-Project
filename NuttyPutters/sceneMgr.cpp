...
void sceneMgr::changeScene(unsigned int newScene, int courseLength, int playerCount, int levelCount, string seed)
{
	...
	case 6:
		...
		gameScene.~gameScene();
		gameScene.Init(windowMgr::getInstance()->win, courseLength, playerCount, levelCount, seed);
		break;
	}
